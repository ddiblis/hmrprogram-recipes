...
async function getList(num) {
    const run = [...Array(num).keys()].forEach(async x => {
        ...
        console.log(getCards(data.data.htmlResult))
    })
}
...
function getCards(html) {
    ...
    $("div.ka-card").each((_, item) => {
        if (item.attribs.onclick) {
            ...
            const recipe = match.groups.recipe
            const company = match.groups.company.toLowerCase().replace(/ /g, "-")
            const url = `https://www.hmrprogram.com/recipes/${company}-diet/${recipe}`
            recipeList[recipe] = { url: url }
        }
    })
    return recipeList
}
